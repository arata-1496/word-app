...
- **PWA対応**: ホーム画面に追加してアプリのように起動できます（マニフェスト・アイコン・サービスワーカーによるオフライン対応つき）。アイコンを作り直す場合は `node scripts/generate-icons.mjs`（`sharp` が必要）。
- **レスポンシブ**: PC（`lg`=1024px以上）とモバイル（iPhone 15 Pro 想定・393px幅）を主対象にブレイクポイントを設定。iPhone の Dynamic Island／ホームインジケータを避けるセーフエリア対応済み。詳細モーダルはモバイルでボトムシート表示になります。
